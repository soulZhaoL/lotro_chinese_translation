...
export function generateTexts(query: Query): TextItem[] {
  ...
  if (query.textId) {
    const textIdValue = Number(query.textId);
    if (!Number.isNaN(textIdValue)) {
      items = items.filter((item) => item.textId === textIdValue);
    }
  }
  if (query.status) {
...
export function generateTextDetailByTextId(fid: string, textId: number): TextItem {
