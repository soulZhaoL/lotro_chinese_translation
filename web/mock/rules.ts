...
export function generateTexts(query: Query): TextItem[] {
  ...
  if (query.textId) {
    items = items.filter((item) => item.textId.includes(query.textId!));
  }
  if (query.status) {
  ...
  if (query.claimed === "false") items = items.filter((item) => !item.isClaimed);

  items = filterByDateRange(items, query.updatedFrom, query.updatedTo);

  const maxLength = 5000;
  return items.map((item) => ({
    ...item,
    sourceText:
      item.sourceText.length > maxLength ? `${item.sourceText.slice(0, maxLength)}...` : item.sourceText,
    translatedText:
      item.translatedText && item.translatedText.length > maxLength
        ? `${item.translatedText.slice(0, maxLength)}...`
        : item.translatedText,
  }));
}
...
export function generateTextDetailByTextId(fid: string, textId: string): TextItem {
  const base = hash(`${fid}-${textId}`);
  const index = (base % 200) + 1;
  const item = createTextItem(index, "detail");
  return {
    ...item,
    fid,
    textId,
...
export function generateDictionary(query: Query) {
  ...
  let items = Array.from({ length: total }, (_, i) => ({
    ...
    termKey: `term_${i}`,
    termValue: `译文_${i}`,
    variantValues: i % 3 === 0 ? [`别名_${i}`, `误译_${i}`] : i % 4 === 0 ? [`历史译法_${i}`] : [],
    correctionVersion: i % 5 === 0 ? 2 : 1,
    appliedCorrectionVersion: i % 5 === 0 ? 1 : 1,
    correctionStatus: i % 5,
    correctionStatusLabel: ["无需纠错", "待纠错", "纠错中", "已完成", "失败"][i % 5],
    correctionLastStartedAt: i % 5 === 2 ? toIso(new Date(Date.now() - i * 20000)) : null,
    correctionLastFinishedAt: i % 5 === 3 || i % 5 === 4 ? toIso(new Date(Date.now() - i * 15000)) : null,
    correctionLastError: i % 5 === 4 ? `纠错失败_${i}` : null,
    correctionUpdatedTextCount: i % 5 === 3 ? i + 2 : i % 5 === 4 ? i : 0,
  ...
  if (query.keyword) {
    items = items.filter((item) =>
      item.termKey.includes(query.keyword!) || item.termValue.includes(query.keyword!)
    );
  }
  if (query.termKey) {
    items = items.filter((item) => item.termKey.includes(query.termKey!));
  }
  if (query.termValue) {
    items = items.filter((item) => item.termValue.includes(query.termValue!));
  }
  if (query.category) {
...
export function generateChanges(textId?: number) {
  ...
  return Array.from({ length: 5 }, (_, i) => ({
    ...
    afterText: `After ${i}`,
    reason: i % 2 === 0 ? "修正" : "补充",
    changedAt: toIso(new Date(Date.now() - i * 120000)),
  }));
}
