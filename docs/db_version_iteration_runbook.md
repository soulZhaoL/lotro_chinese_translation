...
1. 新版本 dat 已解包为 `fid + textId + part + sourceText`。
2. 已计算 `sourceTextHash`（SHA256）。
3. 维护窗口已申请并通知全员。
...
1. `fid + textId + part` 命中
2. 且 `sourceTextHash` 相同（仅对稳定文本建立映射）

## Step 7：迁移 `text_changes`

你确认的策略如下执行：

1. 仅保留 `text_changes.textId` 在映射表中可命中的记录。
2. 将命中记录的 `textId` 批量替换为 `new_id`。
3. 未命中记录直接删除。
...
4. `text_changes` 不存在悬空 `textId`。
5. 核心接口可用：`/texts/parents`、`/texts/children`、`/texts/{textId}`、`/texts/{textId}/translate`。
