...
def _validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    target_table = resolve_env_table_ref(
        require_type(require_key(output_cfg, "targetTable", "output."), str, "output.targetTable"),
        runtime_env,
        "output.targetTable",
    )
...
def _sql_literal(value: Any) -> str:
    ...
    if isinstance(value, (int, float)):
        return str(value)
    # MySQL 字符串字面量转义，避免反斜杠/控制字符导致 SQL 语法中断。
    text = str(value)
    text = text.replace("\\", "\\\\")
...
def _write_insert_sql(handle, table_name: str, columns: List[str], rows: List[List[str]]) -> None:
    ...
    handle.write(f"INSERT INTO {quote_table_ref(table_name)} ({columns_sql}) VALUES\n")
...
if __name__ == "__main__":
    main()
    # python ./tools/version_iteration_tool/step4_generate_text_main_next_insert.py --config ./tools/version_iteration_tool/step4_generate_text_main_next_insert.yaml 
